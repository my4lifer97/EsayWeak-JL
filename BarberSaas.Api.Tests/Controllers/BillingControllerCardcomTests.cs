...
public class BillingControllerCardcomTests : IntegrationTestBase
{
    ...
    private async Task<(string Token, string BusinessId)> RegisterAndLoginBusiness(string email, string slug)
    {
        ...
        return (body!.Token, business.Id);
    }

    [Fact]
    public async Task CheckoutSession_Configured_ReturnsUrlAndPassesBusinessIdAsReturnValue()
    {
        var (token, businessId) = await RegisterAndLoginBusiness("cardcom-checkout@example.com", "cardcom-checkout-shop");
        ...
        Assert.Equal(businessId, call.ReturnValue);
    }

    [Fact]
    public async Task Webhook_VerifiedResultWithToken_ActivatesSubscriptionAndStoresToken()
    {
        var (_, businessId) = await RegisterAndLoginBusiness("cardcom-webhook@example.com", "cardcom-webhook-shop");

        var lowProfileId = Guid.NewGuid().ToString("N");
        Factory.Cardcom.Results[lowProfileId] = new CardcomLowProfileResult(0, null, lowProfileId, "txn-1", "tok-1", 120m, businessId);
        ...
        Assert.Equal("tok-1", business.CardcomToken);
        Assert.Equal(lowProfileId, business.CardcomLastLowProfileId);
        Assert.NotNull(business.CardcomNextChargeAt);
    ...
    {
        var (_, businessId) = await RegisterAndLoginBusiness("cardcom-webhook-dup@example.com", "cardcom-webhook-dup-shop");
        ...
        using (var db = Factory.CreateDbContext())
            nextChargeAfterFirst = (await db.Businesses.SingleAsync(b => b.Id == businessId)).CardcomNextChargeAt;

        var resp = await Client.PostAsync($"/api/billing/webhook?LowProfileId={lowProfileId}", new StringContent(""));

        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
        using var db2 = Factory.CreateDbContext();
        var business = await db2.Businesses.SingleAsync(b => b.Id == businessId);
        Assert.Equal(nextChargeAfterFirst, business.CardcomNextChargeAt);
    }
}
