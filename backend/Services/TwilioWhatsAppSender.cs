...
// Jwt:Secret/CronSecret) sends on behalf of every business -- only which number to send `from` is
// per-business (Business.TwilioNumber, assigned by the platform admin). "Not configured" is still a
// per-call condition -- callers are expected to check business.TwilioNumber is non-null before
...
public class TwilioWhatsAppSender(IConfiguration config) : IWhatsAppSender
{
    public async Task SendAsync(Business business, string toPhone, string message)
    {
        ...
        await MessageResource.CreateAsync(
            from: new Twilio.Types.PhoneNumber($"whatsapp:{business.TwilioNumber}"),
