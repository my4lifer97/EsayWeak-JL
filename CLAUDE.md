...
- `POST/DELETE /api/admin/schedule/overrides[/{id}]` — per-date `WorkingHoursOverride`, bundled into `GET /api/admin/schedule`'s response (`Overrides`) rather than a separate GET. Upsert-by-date (one row per `(BusinessId, Date)`, unique index). Originally built as a standalone "Date-Specific Hours" section that could also *close* a normally-open day — removed after user feedback that it duplicated Blocked Dates (closing a day is just blocking one date, already covered). Now surfaced only as **"Exceptions"** inside the Blocked Dates section of `SchedulePage.tsx`: an active override reopens one specific date with its own hours, for a day that would otherwise sit inside a blocked range (e.g. one working day in the middle of a two-week vacation). Checked first in `AvailabilityService.GetSlotsWithBookingInfo`, before the `DayOfWeek` lookup; when active, it also makes the method skip that date's `BlockedSlot` rows entirely (see below) rather than merely supplying different hours.
- `GET/POST /api/admin/schedule/presets`, `POST .../presets/{id}/apply`, `DELETE .../presets/{id}` — a named snapshot of the 7-day `WorkingHours` template (`SchedulePreset` + `SchedulePresetDay`), saveable and re-applicable later (e.g. "Christmas Hours"). Applying overwrites the standing weekly template via the same per-day upsert helper (`AdminController.UpsertWorkingHours`) `SaveWorkingHours` itself uses — it does **not** touch `WorkingHoursOverride` rows or auto-revert; the owner switches back (or applies a different preset) manually. This exact behavior was confirmed with the business owner over the alternative (applying to a chosen future date range instead) before building it.
...
Before falling back to the weekly `WorkingHours` template, `GetSlotsWithBookingInfo` first checks for a `WorkingHoursOverride` row matching this exact date ("Exceptions" in `SchedulePage.tsx`'s Blocked Dates section) — an active override supplies its own hours for this date AND makes the method skip that date's `BlockedSlot` rows entirely, so an exception day reopens even if it falls inside an otherwise-blocked range; an inactive override (`IsActive = false`) closes an otherwise-open day, same as no active `WorkingHours` row. Only when no override exists for that date does it fall through to the `DayOfWeek` lookup and the normal `BlockedSlot` check.

**Customer-facing block reason**: `AvailabilityService.GetFullDayBlockInfo(businessId, dateStr)` — called by `BookingController`'s two public availability endpoints only when `slots` comes back empty — looks up a full-day `BlockedSlot` for that date (returning `null`, not blocked, if an active exception-day override already reopened it) and returns its optional `Reason`. The response shape is `{ slots, isBlocked, blockedReason }`; `BookingWizard.tsx`'s step 3 shows "The business is closed on this date" (`closedByOwner`) plus the reason text when `isBlocked` is true, instead of the generic `noTimes` message — so a customer sees *why* a date has no times, not just that it doesn't.
