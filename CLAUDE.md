...
- `GET/POST /api/admin/schedule` — working hours (upsert by DayOfWeek). `SchedulePage.tsx`'s Working Hours section is **read-only** (day name, next calendar date, hours or "Closed" per row) — it's a mirror of whatever's currently live, not an editor; the owner edits hours only through the Default schedule preset (below). The endpoint itself is unchanged and still callable directly (used internally by `SchedulePresetService.UpsertWorkingHours`).
