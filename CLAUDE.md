...
- **Schedule presets** — a named 7-day `WorkingHours` template (`SchedulePreset` + `SchedulePresetDay`), edited via `PresetEditorModal.tsx` (a "New Preset"/"Edit Preset" modal reusing the same `WeekHoursEditor` component as the main Working Hours section, so designing a preset feels identical to editing the real weekly template). Days are always caller-supplied — the modal opens pre-filled with a copy of the current live hours when creating, but the owner can freely redesign before saving; nothing is auto-snapshotted from the DB except the automatic backup described below.
  - `GET/POST /api/admin/schedule/presets`, `PUT .../presets/{id}` — list, create (`SaveSchedulePresetRequest{Name,Days}`), rename/redesign an existing one (`UpdateSchedulePresetRequest`).
  - `DELETE /api/admin/schedule/presets/{id}` — blocked (400, not 500) while a `PresetSchedule` still references the preset as either its target or its revert-backup (`Restrict` FK, caught as `DbUpdateException`) — cancel that schedule first.
  - `POST .../presets/{id}/apply` — **immediate, permanent** overwrite of the standing weekly template via `SchedulePresetService.UpsertWorkingHours` (the same per-day upsert `SaveWorkingHours` itself uses). No revert.
  - `POST .../presets/{id}/schedule` (`SchedulePresetRangeRequest{StartDate,EndDate}`) — **temporary, self-reverting**: the template switches to this preset's days on `StartDate` and automatically reverts on the day after `EndDate`. Before switching, `SchedulePresetService.ScheduleForRange` always auto-creates a backup preset (`"Before {name} ({today})"`) snapshotting whatever the template is *right now*, so reverting restores exactly that — not just "whatever the template happens to be by `EndDate`" if it was edited in the meantime. Only one un-reverted `PresetSchedule` per business is allowed at a time (400 if one already exists — cancel it first). A range starting today or earlier applies immediately in the same request, rather than waiting for the next cron sweep.
  ...
  - `DELETE /api/admin/schedule/preset-schedule/{id}` — "close"/cancel a pending or active scheduled change. Pending just deletes the row; active additionally reverts the template to the backup preset first.
  ...
  - This redesign (per-date "exception" overrides removed, then this scheduled-range mechanism added instead) went through two false starts earlier the same session — see git history around `RemoveWorkingHoursOverride` and `AddPresetSchedule` if extending this area again; the owner's actual want was always "reopen/reschedule specific dates," landing here after "block/unblock individual dates is enough" turned out not to cover the recurring holiday-week use case.
