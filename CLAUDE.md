...
- `POST/DELETE /api/admin/schedule/overrides[/{id}]` — per-date `WorkingHoursOverride`, bundled into `GET /api/admin/schedule`'s response (`Overrides`) rather than a separate GET. Bidirectional: can open a normally-closed day with custom hours, or close a normally-open one entirely, for one specific date — checked first in `AvailabilityService.GetSlotsWithBookingInfo`, before the `DayOfWeek` lookup, falling back to the weekly template when no override exists for that exact date. Upsert-by-date (one row per `(BusinessId, Date)`, unique index).
...
Before falling back to the weekly `WorkingHours` template, `GetSlotsWithBookingInfo` first checks for a `WorkingHoursOverride` row matching this exact date — a per-date override that can open a normally-closed day with custom hours, or close a normally-open one entirely (`IsActive = false`, same as no active `WorkingHours` row — returns no slots), without touching the standing weekly schedule. Only when no override exists for that date does it fall through to the `DayOfWeek` lookup.
